/**
 * AIPI Institutional LeadtimeCurveChart Component (ES Module)
 */
...
export class LeadtimeCurveChart {
  ...
  static render(container, props) {
    ...
    const py = (y) => this.M.t + (1 - (y - ymin) / (ymax - ymin)) * (this.H - this.M.t - this.M.b);
    ...
    // 3. X-Axis Labels
    const xLabels = sortedCurve.map((p, i) => {
    ...
    const svgEl = container.querySelector('.leadtime-svg');
    const crosshair = container.querySelector('#curve-crosshair');
