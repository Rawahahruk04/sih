/**
 * AIPI Reusable Enterprise Table Component
 * 
 * High-performance tabular data grid with:
 * - Column sorting (asc/desc with visual indicators)
 * - In-memory filter search
 * - Sticky header
 * - Row selection & keyboard accessibility
 */
...
export interface EnterpriseTableProps<T = any> {
...
export class EnterpriseTable<T extends Record<string, any>> {
  private sortKey: string | null = null;
  private sortDir: 'asc' | 'desc' = 'asc';

  public render(container: HTMLElement, props: EnterpriseTableProps<T>): void {
    let filteredData = [...props.data];

    // 1. Search Filter
    if (props.searchQuery && props.searchQuery.trim() !== '') {
      const q = props.searchQuery.toLowerCase().trim();
      const fields = props.searchFields || (props.columns.map((c) => c.key) as (keyof T)[]);
    ...
    headers.forEach((th) => {
      const handleSort = () => {
        ...
        if (this.sortKey === key) {
          ...
          this.sortKey = key;
          this.sortDir = 'desc'; // Default numerical to descending
        }
    ...
    if (props.onRowClick) {
      const rows = table.querySelectorAll<HTMLTableRowElement>('.table-row.clickable');
      rows.forEach((r) => {
        const handleRow = () => {
