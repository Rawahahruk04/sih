/**
 * AIPI Screen 1: Executive Overview (Headline Inflation Monitor)
 * 
 * Benchmark screen integrating:
 * - GET /health
 * - GET /api/v1/index (daily, weekly, monthly, DoW adjusted, date slicing)
...
import { HeadlineResponse, HealthResponse, PipelineRunModel } from '../types/api.js';
import { htmlToElement } from '../utils/dom.js';
import { fmt } from '../utils/formatters.js';

export interface OverviewPageCallbacks {
  onNotify?: (type: 'success' | 'warning' | 'error' | 'info', title: string, message?: string) => void;
}

export class OverviewPage {
  private container: HTMLElement | null = null;
  private callbacks: OverviewPageCallbacks;

  // Filter & Control States
  private freq: 'daily' | 'weekly' | 'monthly' = 'daily';
  private dowAdjusted = false;
  private dateFrom = '';
  private dateTo = '';

  // Data States
  private headlineData: HeadlineResponse | null = null;
  private headlineAdjData: HeadlineResponse | null = null;
  ...
  private renderContent(): void {
          ...
          <div class="chart-controls-bar">
            ...
            <div class="controls-left">
              <div class="segmented-control" role="tablist" aria-label="Frequency Selector">
                <button class="segmented-control-item ${this.freq === 'daily' ? 'active' : ''}" data-freq="daily">Daily</button>
                <button class="segmented-control-item ${this.freq === 'weekly' ? 'active' : ''}" data-freq="weekly">Weekly</button>
                <button class="segmented-control-item ${this.freq === 'monthly' ? 'active' : ''}" data-freq="monthly">Monthly</button>
              </div>
    ...
    const chartCanvas = page.querySelector<HTMLElement>('#headline-chart-canvas');
    if (chartCanvas) {
  ...
  private attachControlListeners(page: HTMLElement): void {
    // Frequency buttons
    const freqBtns = page.querySelectorAll<HTMLButtonElement>('.segmented-control-item');
    freqBtns.forEach((btn) => {
      btn.addEventListener('click', () => {
        const newFreq = btn.dataset.freq as 'daily' | 'weekly' | 'monthly';
    ...
    // DoW Checkbox
    const dowCheckbox = page.querySelector<HTMLInputElement>('#dow-toggle-checkbox');
    if (dowCheckbox) {
