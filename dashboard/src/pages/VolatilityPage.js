/**
 * AIPI Screen 6: Volatility & Sampling Error (ES Module)
 */
...
export class VolatilityPage {
  constructor(callbacks = {}) {
    ...
    this.windowTable = new EnterpriseTable();
    this.curveTable = new EnterpriseTable();
  ...
  renderContent() {
    ...
    const windowRows = intraday?.by_advance_window
    ...
    const curveRows = samplingError?.curve || [];
          ...
          <div class="col-6">
            <div class="card-container" style="height: 100%;">
              ...
              ${
                ...
                ${intraday.note ? `<p class="text-small" style="color: var(--color-text-tertiary); margin-top: var(--space-10);">${escapeHtml(intraday.note)}</p>` : ''}
              `
                  : `
                <div class="empty-state-container" style="padding: 24px 12px;">
                  <p class="text-body-muted">${escapeHtml(intraday?.note) || 'Single capture slot per day; no intraday spread to measure.'}</p>
                </div>
              `
              }
            </div>
    ...
    const windowMount = page.querySelector('#advance-window-table-mount');
    if (windowMount && windowRows.length > 0) {
      ...
      this.windowTable.render(windowMount, {
        ...
        ariaLabel: 'Intraday coefficient of variation by advance-purchase window table'
      });
    }

    // Mount sampling-error curve chart + table
    if (curveRows.length > 0) {
      const chartCanvas = page.querySelector('#sampling-curve-chart-canvas');
      if (chartCanvas) {
        const seriesList = [
          {
            id: 'mae',
      ...
      const curveMount = page.querySelector('#sampling-curve-table-mount');
      if (curveMount) {
        const columns = [
          {
            key: 'days_per_month',
        ...
        this.curveTable.render(curveMount, {
          ...
          ariaLabel: 'Sparse-sampling error by collection frequency table'
        });
      }
    }
  }
