...
export class ApiExplorerPage {
  constructor(callbacks = {}) {
    ...
    this.health = null;
    this.lastHealthLatency = null;
    this.lastHealthUpdated = null;
    this.routeFallback = 'DEL-BOM';
    this.inspector = null;
    this.openapiSpec = null;
    this.table = new EnterpriseTable();
  }
  ...
  async executeSelected() {
    ...
    const form = this.container?.querySelector('#api-request-form');
  ...
  async loadRouteFallback() {
    try {
      const meta = await api.getRouteMetadata();
            ...
            <section class="card-container api-sticky-panel">
              ...
              <div class="api-filter-row">
                <label class="filter-input-group" for="api-search-input">
                  <span>Search</span>
                  <input class="filter-input api-search-input" id="api-search-input" type="search" value="${escapeHtml(this.searchQuery)}" />
                </label>
  ...
  renderStatusCard(label, value, hint) {
    return `
      <div class="stat-card">
  ...
  renderContractBlock(title, values) {
    return `
    ...
    return `
      <section class="card-container" aria-labelledby="live-console-heading">
        ...
        <div class="api-url-preview" aria-live="polite">
          ...
          <code>${escapeHtml(this.previewUrl(endpoint))}</code>
        </div>
      </section>
    `;
  }

  renderParameterControl(param, endpoint) {
    ...
    return `
      <label class="filter-input-group api-param-control">
        <span>${label}</span>
        <input class="filter-input" type="${inputType}" name="${param.name}" value="${escapeHtml(current)}" aria-label="${label}" />
      </label>
    `;
  }
    ...
    return `
      <section class="card-container" aria-labelledby="schema-heading">
        ...
        <details class="api-schema-tree" open>
          ...
          <pre id="schema-json">${escapeHtml(JSON.stringify(schema, null, 2))}</pre>
        </details>
      </section>
    `;
  }
    ...
    return `
      <section class="card-container" aria-labelledby="response-heading">
        ...
        <details class="api-schema-tree" open>
          ...
          <pre>${escapeHtml(JSON.stringify(result?.headers || {}, null, 2))}</pre>
        </details>
        <details class="api-schema-tree" open>
          <summary>Formatted JSON</summary>
          <pre id="response-json">${escapeHtml(JSON.stringify(result?.json || { status: 'awaiting_request' }, null, 2))}</pre>
        </details>
      </section>
    `;
  }
  ...
  renderDeveloperNotes() {
    ...
    return `
      <section class="card-container" aria-labelledby="dev-notes-heading">
        ...
        <ul class="api-notes-list">${notes.map((n) => `<li>${escapeHtml(n)}</li>`).join('')}</ul>
      </section>
    `;
  }

  renderDirectoryTable(root) {
    const mount = root.querySelector('#api-directory-table');
    if (!mount) return;
    ...
    this.table.render(mount, {
      ...
      ariaLabel: 'Verified backend endpoint directory table'
    });
  }
  ...
  attachListeners(root) {
    ...
    root.querySelector('#run-error-demo-btn')?.addEventListener('click', () => {
      ...
      this.renderContent();
      const freq = this.container?.querySelector('select[name="freq"]');
      const dow = this.container?.querySelector('input[name="dow_adjusted"]');
  ...
  buildUrl(path, endpoint, formData) {
  ...
  async copyElementText(selector, message) {
