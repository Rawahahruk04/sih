/**
 * AIPI Screen 6: Methodology & Governance Dossier (ES Module)
 */
...
export class MethodologyPage {
  ...
  renderContent() {
    ...
    const quarantineRows = Object.entries(quarantineReasons).map(([reason, count]) => ({
      reason,
      count: count,
      share_pct: totalQuarantined > 0 ? `${(((count) / totalQuarantined) * 100).toFixed(1)}%` : '0.0%'
    }));
            ...
            <div>
              ...
              <div style="display: flex; gap: 8px; flex-wrap: wrap; margin-top: 12px;">
                <span class="badge badge-neutral" style="font-family: var(--font-family-mono);">
                  FINGERPRINT: ${typeof m.fingerprint === 'string' ? m.fingerprint.slice(0, 16) : `GEKS W=${m.fingerprint?.geks_window_days || 25}d · MAD k=${m.fingerprint?.mad_trim_k || 3.5}`}
                </span>
    ...
    const quaranMount = page.querySelector('#quarantine-table-mount-point');
    if (quaranMount) {
      const qCols = [
        {
          key: 'reason',
    ...
    const weightMount = page.querySelector('#weights-table-mount-point');
    if (weightMount) {
      const wCols = [
        {
          key: 'route_code',
