/**
 * AIPI Screen 3: Route Detail Inspector
 * 
 * Deep-dive single-sector trajectory inspector consuming:
 * - GET /api/v1/index/routes/{route_code}
 */
...
import { IndexPoint, RouteResponse } from '../types/api.js';
...
export class RouteDetailPage {
  ...
  private callbacks: RouteDetailCallbacks;
  private routeCode: string;

  // Data States
  private routeData: RouteResponse | null = null;
  private loading = false;
  private error: ApiError | null = null;

  private tableInstance = new EnterpriseTable<IndexPoint>();
  private abortController: AbortController | null = null;

  constructor(routeCode: string, callbacks: RouteDetailCallbacks) {
    this.routeCode = routeCode;
    this.callbacks = callbacks;
  }

  public setRouteCode(code: string): void {
    this.routeCode = code;
    this.fetchData();
  }
  ...
  private renderContent(): void {
    ...
    const chartCanvas = page.querySelector<HTMLElement>('#route-detail-chart-canvas');
    if (chartCanvas) {
    ...
    const tableMount = page.querySelector<HTMLElement>('#route-timeline-table-mount');
    if (tableMount) {
      const reversedPoints = [...points].reverse(); // Newest first

      const columns: TableColumn<IndexPoint>[] = [
        {
          key: 'date',
          label: 'Date',
          width: '140px',
